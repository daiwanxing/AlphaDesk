...
| Path | Role |
|------|------|
| `colors_and_type.css` | 色与字体 token + 工具类 |
| `tokens.css` | 间距 / 半径 / 阴影 / 布局 |
| `preview/` | 分项审查卡 |
...
生成新产物时：粘贴 `colors_and_type.css`（及需要时 `tokens.css`）到首个 `<style>`，按本文件组件表与 `ui_kits/app` 对齐结构。
