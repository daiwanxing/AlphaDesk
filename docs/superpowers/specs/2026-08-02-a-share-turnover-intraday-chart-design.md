...
| 图表库        | **Apache ECharts**（`echarts/core` + 按需 `LineChart` / 组件 / renderer；ESM tree-shake）。统一终端后续图表基建；不用手写 SVG / uPlot 作主路径。午休不占宽用 category/ordinal 轴或自定义 `xAxis` 映射实现（§4.2） |
...
- 为本页再引入第二套图表库（uPlot / 自绘 SVG 主路径等）；ECharts 按需导入即可
...
2. 前端类型与 API 适配
3. 前端接入 ECharts 按需包（共享 `src/shared/charts` 或 feature 内封装一层）
4. TurnoverBoard：KPI + 分时图替换三卡主视觉
5. 会话文案与降级态
6. 测试与手动盘中/休市验收

### 图表实现备注（ECharts）

- 依赖：`echarts`（官方类型内置；用 `echarts/core` + `LineChart` + `GridComponent` + `TooltipComponent` + `CanvasRenderer` 等按需 `use`）
- 系列：今日 `line` + `areaStyle`（冷蓝）；昨日 `line`（橙、无面积）
- 动画：可用默认/轻量入场（`animationDuration` 克制）；盘中 15s 刷新用 `setOption`，避免每次整图销毁重建
- 午休轴：X 为交易分钟 ordinal（拼接上午+下午），刻度只标关键时刻；与 §4.2 一致
